...
void GPIOPortF_Handler(void)
{
   ...
   // printf("%x\n",GPIO_PORTF_RIS_R);
    ...
    {
        endTick = TIMER4_TAR_R;         // read current timer value (end)

          TIMER4_CTL_R &= ~0x00000001;    // stop timer
          GPIO_PORTF_ICR_R = 0x10;          // acknowledge flag for PF4
          deltaTick = startTick- endTick -30000 ;
         ...
         // printf("\n pulse width %ld\n",deltaTick);
          ...
          if(deltaTick<200000)
          {
              //TIMER0_CTL_R = 0x0000;            /* disable Timer before initialization */
          ...
          else
          {
              ...
              TIMER0_TAMATCHR_R = 0x4E20;
              TIMER0_TBMATCHR_R = 0x4E20;
          }
    }
